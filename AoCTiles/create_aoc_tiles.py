...
def parse_leaderboard(html: str) -> dict[str, DayScores]:
...
def generate_day_tile_image(day: str, year: str, languages: list[str], day_scores) -> Path:
...
def handle_day(day: int, year: int, day_path: Path, html: HTML, day_scores):
    languages = []
    ...
    with html.tag("a", href=str(solution_file_path).replace("\\", "/")):
        html.tag("img", closing=False, src=str(day_graphic_path).replace("\\", "/"), width=TILE_WIDTH_PX)
